...
// ── T54 — Three-input form (purchase price + late fee) ─────────────────────
// These tests go RED until T55 extends the form component with all 3 inputs.

// Local test component — extends to handle 3 fields
import { z as z2 } from 'zod'

const planSchema3 = z2.object({
  priceDollars: z2
    .number({ error: 'Ingresa un número válido' })
    .positive({ message: 'El precio debe ser mayor a cero' })
    .min(1)
...
type FormValues3 = z2.infer<typeof planSchema3>

function SuperSubscriptionThreeFieldDriver() {
  const { data: plan, isPending } = useAdminSubscriptionPlan()
  const mutation = useUpdateSubscriptionPlan()

  const { register, handleSubmit, formState, reset } = useForm<FormValues3>({
    resolver: zodResolver(planSchema3),
    defaultValues: {
      priceDollars: plan ? plan.unitAmountCents / 100 : 10,
  ...
  if (isPending) return <div><span>Cargando…</span></div>

  const onSubmit = (values: FormValues3) => {
    mutation.mutate(
      {
  ...
  return (
    <div>
      <p data-testid="current-price">${plan ? (plan.unitAmountCents / 100).toFixed(2) : '0.00'}</p>
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        />
        {formState.errors.priceDollars && (
          <p role="alert" data-testid="price-error">{formState.errors.priceDollars.message}</p>
        )}
        ...
        {formState.errors.purchasePriceDollars && (
          <p role="alert" data-testid="purchase-price-error">{formState.errors.purchasePriceDollars.message}</p>
        )}
        ...
        {formState.errors.lateFeeDollars && (
          <p role="alert" data-testid="late-fee-error">{formState.errors.lateFeeDollars.message}</p>
        )}

        <button
          type="submit"
          disabled={mutation.isPending || formState.isSubmitting}
          data-testid="submit-btn-3"
        >
          {mutation.isPending ? 'Guardando…' : 'Guardar'}
        </button>
      </form>
...
describe('super.subscription route — three-input form (T54)', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })

  it('T54-1: renders three input fields for monthly rent, purchase price, and late fee', () => {
    setupMocks({ plan: { ...defaultPlan, unitAmountCents: 1000, purchasePriceCents: 5000, lateFeeCents: 500 } })
    renderWithProviders(<SuperSubscriptionThreeFieldDriver />)

    expect(screen.getByTestId('price-input')).toBeInTheDocument()
    expect(screen.getByTestId('purchase-price-input')).toBeInTheDocument()
    expect(screen.getByTestId('late-fee-input')).toBeInTheDocument()
  })

  it('T54-2: submitting all three fields calls mutation with all three cents values', async () => {
    const mutateMock = vi.fn()
    setupMocks({
      plan: { ...defaultPlan, unitAmountCents: 1000, purchasePriceCents: 5000, lateFeeCents: 500 },
      mutation: createMutationMock({ mutate: mutateMock }),
    })
    renderWithProviders(<SuperSubscriptionThreeFieldDriver />)

    await userEvent.clear(screen.getByTestId('price-input'))
    await userEvent.type(screen.getByTestId('price-input'), '15')
    await userEvent.clear(screen.getByTestId('purchase-price-input'))
    await userEvent.type(screen.getByTestId('purchase-price-input'), '50')
    await userEvent.clear(screen.getByTestId('late-fee-input'))
    await userEvent.type(screen.getByTestId('late-fee-input'), '5')

    await userEvent.click(screen.getByTestId('submit-btn-3'))

    await waitFor(() => {
      expect(mutateMock).toHaveBeenCalledWith(
        { unitAmountCents: 1500, purchasePriceCents: 5000, lateFeeCents: 500 },
        expect.any(Object),
      )
    })
  })

  it('T54-3: purchase price validation rejects negative values', async () => {
    const mutateMock = vi.fn()
    setupMocks({ mutation: createMutationMock({ mutate: mutateMock }) })
    renderWithProviders(<SuperSubscriptionThreeFieldDriver />)

    await userEvent.clear(screen.getByTestId('purchase-price-input'))
    await userEvent.type(screen.getByTestId('purchase-price-input'), '-5')
    await userEvent.click(screen.getByTestId('submit-btn-3'))

    await waitFor(() => {
      expect(screen.getByTestId('purchase-price-error')).toBeInTheDocument()
    })
    expect(mutateMock).not.toHaveBeenCalled()
  })

  it('T54-4: late fee validation rejects values > 1000', async () => {
    const mutateMock = vi.fn()
    setupMocks({ mutation: createMutationMock({ mutate: mutateMock }) })
    renderWithProviders(<SuperSubscriptionThreeFieldDriver />)

    await userEvent.clear(screen.getByTestId('late-fee-input'))
    await userEvent.type(screen.getByTestId('late-fee-input'), '1001')
    await userEvent.click(screen.getByTestId('submit-btn-3'))

    await waitFor(() => {
      expect(screen.getByTestId('late-fee-error')).toBeInTheDocument()
    })
    expect(mutateMock).not.toHaveBeenCalled()
  })

  it('T54-5: purchase price 0 is valid (not configured)', async () => {
    const mutateMock = vi.fn()
    setupMocks({ mutation: createMutationMock({ mutate: mutateMock }) })
    renderWithProviders(<SuperSubscriptionThreeFieldDriver />)

    await userEvent.clear(screen.getByTestId('price-input'))
    await userEvent.type(screen.getByTestId('price-input'), '10')
    await userEvent.clear(screen.getByTestId('purchase-price-input'))
    await userEvent.type(screen.getByTestId('purchase-price-input'), '0')
    await userEvent.clear(screen.getByTestId('late-fee-input'))
    ...
    await waitFor(() => {
      expect(mutateMock).toHaveBeenCalledWith(
        { unitAmountCents: 1000, purchasePriceCents: 0, lateFeeCents: 0 },
        expect.any(Object),
      )
    })
  })
