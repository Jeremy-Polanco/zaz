...
App mobile de DashGo. Expo 55 · React Native · NativeWind · TanStack Query.
...
- **Referral landing:** `dashgo://r/ABC12DEF` o `https://dashgo.app/r/ABC12DEF` → `src/app/r/[code].tsx` → precarga promotor via `usePromoterByCode`, pasa código al login.

Scheme: `dashgo`. Configurado en `app.json` + `StripeProvider.urlScheme="dashgo"`.
