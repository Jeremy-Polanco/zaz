...
describe('PaymentReconciliationCron', () => {
  ...
  it('advances an order whose hold Stripe already authorized (the missed-webhook case)', async () => {
    ordersRepo.find.mockResolvedValue([stuck('order-1', 'pi_held')]);
    ...
    expect(result).toEqual({ scanned: 1, authorized: 1, released: 0 });
  });

  it('also stamps paidAt when Stripe reports the intent already captured', async () => {
    ordersRepo.find.mockResolvedValue([stuck('order-1', 'pi_done')]);
  ...
  it('reverts an order whose intent was canceled at Stripe (customer can retry)', async () => {
    ordersRepo.find.mockResolvedValue([stuck('order-1', 'pi_gone')]);
    ...
    expect(result).toEqual({ scanned: 1, authorized: 0, released: 1 });
  });

  it('leaves an order alone while the customer has not finished paying', async () => {
    ordersRepo.find.mockResolvedValue([stuck('order-1', 'pi_open')]);
    ...
    expect(result).toEqual({ scanned: 1, authorized: 0, released: 0 });
  });

  it('keeps going when Stripe fails for one intent', async () => {
    ordersRepo.find.mockResolvedValue([
      stuck('order-1', 'pi_broken'),
      stuck('order-2', 'pi_held'),
    ]);
    ...
    expect(payments.markAuthorizedByIntentId).toHaveBeenCalledWith('pi_held');
    expect(result).toEqual({ scanned: 2, authorized: 1, released: 0 });
  });
  ...
  it('is a no-op when Stripe is not configured', async () => {
    ...
    expect(result).toEqual({ scanned: 0, authorized: 0, released: 0 });
  });
