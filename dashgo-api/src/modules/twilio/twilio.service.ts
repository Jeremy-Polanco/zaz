...
export class TwilioService {
  ...
  constructor(private readonly config: ConfigService) {
    ...
    if (accountSid && apiKeySid && apiKeySecret && this.fromNumber) {
      ...
      this.logger.log(
        `Twilio initialized (API Key auth, SMS from ${this.fromNumber}). ` +
          `WhatsApp OTP is handled by WhatsAppService (Meta Cloud API), not Twilio.`,
      );
    } else {
      this.client = null;
      const missing = [
  ...
  async sendOrderNotificationSms(order: Order): Promise<void> {
    const numbers = this.config.get<string[]>('ORDER_SMS_NOTIFY_NUMBERS') ?? [];
