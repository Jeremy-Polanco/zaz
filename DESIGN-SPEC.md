...
A user who promotes DashGo to friends in exchange for commission. Has their own dashboard tab group. Sees referral code, referred customer list, commission balance, payout history. Can also order as a customer (catalog access). Promoters do NOT have a credit account or subscription.
...
### 3.1 Logo
The DashGo wordmark reads **Dash ⚡ Go** — two words in white Inter Tight Bold separated by a single **orange lightning bolt** standing in for the space between them. The bolt is the speed signature; it is also used standalone (in `<BoltIcon>`) wherever the UI needs to telegraph delivery, ETA, or fast action. The full mark renders on a black or near-black surface; on light surfaces the type flips to ink and the bolt stays orange. The mark always reads "DashGo" — never "ZAS", "ZAZ", or "Z⚡Z" (those are legacy brand names from earlier iterations).

**Standalone glyph (favicon, app icon, small marks)**: just the orange lightning bolt on a black square. The bolt is the brand's most reduced form and must be recognizable at 16px.
...
| `paper-elev` | `#FFFFFF` | Highest elevation (modal sheets, drawers) |
| `ink` | `#1A1530` | Primary text, dark icons, neutral chrome (spinners, dividers) |
| `ink-soft` | `#4A4566` | Body text, secondary labels |
| `ink-muted` | `#6B6488` | Tertiary, captions, placeholders |
| `brand` | `#000000` | DashGo black — primary action, branded surfaces, dark posters |
| `brand-dark` | `#0A0A0A` | Pressed states, text on orange accent |
| `brand-light` | `#FFF5E6` | Soft orange-tinted callouts, subtle highlight backgrounds |
| `accent` | `#FF8000` | DashGo orange — the lightning-bolt color, CTAs, badges |
| `accent-dark` | `#CC6600` | Pressed accent |
| `accent-light` | `#FFE0BF` | Pale orange wash (notifications, banner backgrounds) |
...
- **Orange is the CTA color**. `bg-accent` (orange) with `text-brand-dark` (near-black) is the primary CTA on every important screen — login submit, checkout confirm, "Cotizar", "Autorizar pago". The orange is loud and confident; use it for the action that matters most on each screen, not for chrome.
- **Black is the brand surface**, not a CTA. `bg-brand` (black) is the dark poster header, the sticky cart bar, the profile avatar circle, and the dashboard nav strip — moments where the UI goes "pressed-in" and signals identity. Black surfaces always pair with `text-paper` (off-white) for the type and `text-accent` (orange) ONLY when calling out a price, a status, or the bolt. Pure-black buttons should be rare — prefer the orange accent for the primary action.
- **Ink is for type and chrome**, never for action. `bg-ink` is reserved for the cart sticky bar in catalog (the one moment the cart needs to feel separate from the brand-black header). `text-ink` is the default body text; `color="#1A1530"` is the default spinner color. Don't promote ink to a button.
- **Orange text on light backgrounds is legible** (unlike the old yellow) — but use it sparingly. `text-accent` is reserved for emphasis: prices on offer, the bolt-icon next to ETAs, links inside dark posters. Don't paint paragraphs orange.
...
**Layout pattern**: a "poster" header (black `bg-brand` block with hero text, eyebrow, italic accent word, big italic step number "01", subtitle) followed by a form section on a slightly elevated `paper-deep` surface.

**Step 1 — Phone**:
- Eyebrow: "Ingresar"
- Field: telephone (with placeholder `+18091234567`)
- CTA: "Enviar código →" (orange accent button, dark text)
...
5 tabs always visible. SF Symbols icons + uppercase 10px labels. Active tab uses **DashGo black** for both icon tint and label.
...
- Horizontal-scrolling chip row: "Todos" + each category. Active chip is orange with dark text; inactive is bordered with muted text.
...
- Top-left of image: an OFERTA badge (orange with dark uppercase text) when an offer is active.
...
- Action: a thin **outlined** AGREGAR button (border + transparent fill + ink text). When quantity > 0, it transforms into a tri-element: minus button, count, plus button. **The AGREGAR is intentionally muted** — orange CTAs are reserved for the checkout, not for every row.

**Sticky cart bar** (appears at bottom only when items in cart):
- Floats above the tab bar.
- `bg-ink` (dark) with `text-paper` content.
- Left: count badge (small orange square, dark number) + "EN CARRITO" label.
- Below count: total in big tabular-numbers, white.
- Right: orange CTA "IR AL CHECKOUT →" with dark text.

**Pull to refresh**: yes. Loading indicator is DashGo black.
...
5. **Resumen** — itemized list:
   ...
   - **Total estimado**: big number, DashGo black
   - CTA: "Confirmar pedido →" (full-width orange accent button with dark text)
...
- Primary CTA below cards (only if balance owed > 0): "Pagar ahora →" orange accent → routes to credit-pay screen.
...
Promoters are people who recommend DashGo in exchange for a 10% commission on referred customers' delivered orders.
...
- Two buttons: "Copiar código", "Compartir link" (uses native share sheet, includes `https://dashgo.com/r/{code}`).
...
- **In browser**: shows a one-screen landing — promoter's first name, lightning bolt visual, "Te invito a usar DashGo" message, big CTA "Crear mi cuenta →" that takes them to login with the code attached.
...
The web admin lives at the root domain (e.g., `app.dashgo.com`). Super admins log in with their phone OTP just like clients. After login, non-super-admin users are redirected away.

### 6.1 Top navigation
A persistent horizontal bar with the DashGo wordmark on the left (subtle orange dot before "AGUA · NEW YORK" eyebrow) and 6 nav links on the right:
**Ruta · Productos · Categorías · Promotores · Crédito · Reparto** + "Salir" log out button.

The active link has a thin orange underline (this is one of the rare valid uses of `bg-accent` as visual decoration — but applied as a 2px underline, not a fill).
...
- **Acciones** (varies by status):
  - `pending_quote`: orange CTA button "Cotizar →" → opens QuoteDrawer.
  ...
  - `delivered`: brand-black link "Ver factura ↗".
...
- Footer: "Cotizar →" submit (DashGo black), "Cancelar" ghost.
...
- Top-right page action: "+ Nuevo producto" — orange accent CTA with dark text.
...
- Discounted prices: original price strikethrough next to new price; new price in `text-ink` (high contrast — never orange).
...
3. Server detects "primer ingreso" → app reveals an inline name field with brand-black accent border. User enters first + last name.
...
|---|---|---|---|
| **Accent** (primary CTA) | `bg-accent` (orange) | `text-brand-dark` | Main action of a screen — submit, confirm, place order |
...
### 10.8 Loading
Spinner color: `ink` (#1A1530) in light theme — neutral chrome, not branded. Position: centered in the screen for full-page loads. Inline within rows for granular operations.
...
- Multi-tenant operator selection (currently one DashGo deployment per market).
